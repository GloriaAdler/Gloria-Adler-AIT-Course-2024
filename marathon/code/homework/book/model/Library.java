...
public class Library {
    ...
    //fields
    private Book1[] books; //массив это сущьность объектов. Сколько потенциально может быть книг в библиотеке, длина массива никогда не может показать, сколько индексов в реальности занято, так как все не занятые индексы будут заполнены null - пустыми ссылками
    ...
    //CRUD methods
//boolean addBook
    public boolean addBook (Book1 book){
        //negative cases
        if (book == null){
            return false;
        }
...
//Book findBook
    public Book1 findBook(long isbn){
        for (int i = 0; i < size; i++) {
...
//removeBook
    public Book1 removeBook (long isbn){
        for (int i = 0; i < size; i++) {
            if (books[i].getIsbn() == isbn){
                Book1 victim = books [i];
