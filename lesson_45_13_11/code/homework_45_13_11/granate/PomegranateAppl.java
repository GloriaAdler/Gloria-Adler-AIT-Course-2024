package homework_45_13_11.granate;
...
//							.count();
    ...
    private static List<Box> getRandomBoxes() {
        int quantity = MIN + random.nextInt(MAX - MIN + 1); // задумали случ. число в диапазоне
    ...
    private static List<Pomegranate> getGranates(){
        int quantity = MIN + random.nextInt(MAX_GRANATE - MIN_GRANATE + 1); // задумали случ. число в диапазоне
        // создаем стрим случайных целых чисел oт 0 до quantity, это аналог i++
        return IntStream.range(0, quantity)
    ...
    private static List<Seed> getSeeds(){
        int quantity = MIN + random.nextInt(MAX_SEED - MIN_SEED + 1); // задумали случ. число в диапазоне
